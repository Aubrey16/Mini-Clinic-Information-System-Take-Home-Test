...
└── modules/               # one folder per feature
    ...
    └── dashboard/         # DONE — Turn 9 (counters dari v_dashboard_today)
...
## 11. Implementation Roadmap (Turns 4–9) — ALL DONE
...
Plus master data endpoints `GET /polis` and `GET /doctors`.

**Turn 7 — Queues (DONE)**
`POST /queues` from a registration (number `A001` auto-generated by trigger).
`PUT /queues/call-next` calls the first `menunggu` queue.
`PUT /queues/{id}/call` sets status `dipanggil` + `called_at`.
`PUT /queues/{id}/status` for the other transitions and timestamps
(`started_at`, `finished_at`), synced with the registration status in one
transaction. Tables: `queues`.

**Turn 8 — Medical records (DONE)**
...
`examined_at DESC`. `GET /medical-records` lists all records with search +
pagination. `POST /prescriptions` + `GET /prescriptions/{id}`. Tables:
`medical_records`, `medical_actions`, `prescriptions`, `prescription_items`.

**Turn 9 — Dashboard (DONE)**
`GET /dashboard` returns `SELECT * FROM v_dashboard_today`
(total patients, patients today, queues today, waiting, finished) plus
today's activity summary (check-ins, in-examination, active doctors) and
the 5 most recent queues.
